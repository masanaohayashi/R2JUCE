...
class R2IcloudDriveProvider : public R2CloudStorageProvider {
 public:
  /**
   * @brief Default constructor, consistent with other providers.
   */
  R2IcloudDriveProvider();

  ~R2IcloudDriveProvider() override;
  
  /**
   * @brief Sets the iCloud container ID and initializes the connection.
   * @param containerId The identifier of the iCloud container to use.
   */
  void setContainerId(const juce::String& containerId);
